...
      member.user.fetch(),
      new Promise((*, reject) => setTimeout(() => reject(new Error('Profile fetch timeout')), 3000))
    ]);
...
      .setColor('#ff0000')
      .setDescription(
        Prefix commands use ${PREFIX} Slash commands: help checkbio forcebio config setcustomrole setbiostring setroleannouncechannel resetdb reactionrole rank leaderboard setxpboost resetxp setwall setlevelrole removelevelrole listlevelroles postvoicepanel voicepanel setjtc
      );
...
    const last = cfg.*lastXp[message.author.id] || 0;
    if (now - last > 5000) {
      cfg.*lastXp[message.author.id] = now;
...
        .setDescription(**welcome to darween’s closet** Read these before you start posting weird shii: **1. Don’t Be a Jerk (It’s Free to Be Nice)** Racism, homophobia, transphobia, sexism, and all those “-isms” = instant L. We're here to vibe, not to ruin lives. **2. Keep Your Pants On (Literally & Digitally)** No NSFW, no "accidental" sauce, no thirst traps. This ain’t OnlyFans, fam. **3. No Spam, Ma’am** Don’t flood the chat with walls of nonsense, links to your mixtape, or "sub for sub." We get it. You're built different. Still, no. **4. Wrong Channel? Go to Jail (jk, but fr tho)** Memes go in #memes. Music in <#1382024376635097292>. If you drop your SoundCloud link in <#1438764300272402452>, we will clown you. **5. No Drama, Save It for Your Therapist** This is a Discord server, not Keeping Up With the Krusty Krew. Sort it out privately or with a mod. **6. Mods Are Not Your Enemies (Unless... :eyes:)** Listen to the mod team. They’re not power-hungry, they just drank too much coffee and now they run the server. **7. No Fake Flexing or Catfishing** Don’t pretend to be someone you're not. Especially if that someone is Drake. You're not Drake. **8. Protect Ya Neck (and Your Personal Info)** Don’t share your number, address, bank pin, or your Netflix password. (Unless you're offering :eyes:) **9. Speak English, Mostly** This server runs on English. Throw in some spice if you want, but don’t go full Shakespeare or alien language. **10. Be Funny, Not a Menace** Sarcasm? Great. Dark humor? Maybe. Just don’t cross the line into “Why are you like this?” :warning: Break these rules and staff can mute, kick or ban you if needed.)
...
        .setColor('#ff0000')
        .setDescription(
          Prefix: ${PREFIX} Admin: ${PREFIX}setwelcomechannel ${PREFIX}setwelcomemsg ${PREFIX}setwelcomeimage ${PREFIX}setruleschannel ${PREFIX}setrulesmsg ${PREFIX}setannouncechannel ${PREFIX}announce ${PREFIX}setautorole ${PREFIX}setcustomrole ${PREFIX}setbiostring ${PREFIX}giverole ${PREFIX}createrole ${PREFIX}setspotify ${PREFIX}setsocial ${PREFIX}setroleannouncechannel ${PREFIX}checkbio ${PREFIX}forcebio ${PREFIX}resetdb ${PREFIX}reactionrole ${PREFIX}setxpboost ${PREFIX}resetxp ${PREFIX}setwall ${PREFIX}setlevelrole ${PREFIX}removelevelrole ${PREFIX}listlevelroles ${PREFIX}setjtc ${PREFIX}postvoicepanel User: ${PREFIX}rules ${PREFIX}testwelcome ${PREFIX}config ${PREFIX}socials ${PREFIX}rank ${PREFIX}leaderboard ${PREFIX}voicepanel
        );
